...
- 2026-09-19: Made routine Blazor circuit recovery unobtrusive by retrying immediately with progressive backoff and withholding the reconnect banner for five seconds unless recovery fails.
- 2026-09-19: Adopted `wwwroot/yala.png` as the YALA brand mark across the app shell and setup screens, preserving the existing slight angle and using the static-asset manifest for cache-safe browser delivery. Container builds also normalize binary images represented as `base64:` text by remote Git-stack staging before publishing static assets.
